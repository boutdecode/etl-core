...
class JsonFileExtractStep extends AbstractExtractorStep
{
    ...
    public function extract(mixed $source, Context $context, array $configuration = []): array
    {
        ...
        $pointer = $configuration['pointer'] ?? $this->configuration['pointer'] ?? $this->pointer;
        $pointerValue = is_string($pointer) ? $pointer : null;

        $result = [];

        data_frame()
            ->read(from_json($source, $pointerValue))
            ->collect()
